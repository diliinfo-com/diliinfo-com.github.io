...
// .wrangler/tmp/bundle-yLq05n/checked-fetch.js
...
function createJWT(payload, secret) {
  ...
  const payloadStr = btoa(JSON.stringify({ ...payload, exp: Date.now() + 24 * 60 * 60 * 1e3 }));
...
app.get("/api/admin/stats", adminAuth, async (c) => {
  try {
    ...
    `).bind(Date.now() - 7 * 24 * 60 * 60 * 1e3).all();
    return c.json({
      summary: {
...
app.post("/api/applications/guest", async (c) => {
  try {
    ...
    await c.env.DB.prepare(`
      ...
      VALUES (?, ?, 1, TRUE, ?, ?)
    `).bind(applicationId, sessionId, Math.floor(Date.now() / 1e3), Math.floor(Date.now() / 1e3)).run();
    await c.env.DB.prepare(`
...
app.put("/api/applications/:id/step", async (c) => {
  try {
    ...
    let params = [step, Math.floor(Date.now() / 1e3)];
    if (phone && step === 1) {
      updateQuery += ", phone = ? ";
      params.push(phone);
    }
    if (step === 2 && data?.idNumber && data?.realName) {
      ...
      params.push("submitted", Math.floor(Date.now() / 1e3));
    } else if (step === 10) {
      updateQuery += ", status = ?, approved_at = ?, approval_amount = ? ";
      params.push("approved", Math.floor(Date.now() / 1e3), 5e4);
...
var userAuth = /* @__PURE__ */ __name(async (c, next) => {
  const authHeader = c.req.header("Authorization");
  if (!authHeader?.startsWith("Bearer ")) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  ...
  try {
    ...
    if (!user) {
      ...
      await c.env.DB.prepare(`
        ...
        VALUES (?, ?, TRUE, 'active', ?)
      `).bind(userId, phone, Math.floor(Date.now() / 1e3)).run();
      await c.env.DB.prepare(`
        INSERT INTO user_activities (id, user_id, activity_type, activity_data)
...
// .wrangler/tmp/bundle-yLq05n/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yLq05n/middleware-loader.entry.ts
